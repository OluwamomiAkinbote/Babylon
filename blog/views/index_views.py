...
class TrendAPIView(APIView):
    def get(self, request):
        try:
            ...
            trending = Category.objects.get(name='Trending')
            subcategories = trending.subcategories.all()
            category_ids = [trending.id] + [sub.id for sub in subcategories]
        ...
        return Response({
            "trending_posts": BlogPostSerializer(posts, many=True).data,
...
class WorldAPIView(APIView):
    def get(self, request):
        try:
            # Get parent category and its subcategories
            world_news = Category.objects.get(name='World')
            subcategories = world_news.subcategories.all()
            category_ids = [world_news.id] + [sub.id for sub in subcategories]
        ...
        return Response({
            "world_news_posts": BlogPostSerializer(posts, many=True).data,
            # Optional: Include subcategory breakdown
            "subcategories": [sub.name for sub in subcategories]
        })
...
class MainExclusiveAPIView(APIView):
    def get(self, request):
        ...
        breaking_queryset = BlogPost.objects.filter(
            category__id__in=breaking_category_ids
        ).order_by('-date')[:20]

        # Combine posts for random main post selection
        combined_posts = list(politics_queryset) + list(breaking_queryset)
        ...
        return Response({
            ...
            'breaking_posts': BlogPostSerializer(breaking_queryset, many=True, context={'request': request}).data,
        })
